...
#J = dpsi/dy * domega/dx - dpsi/dx * domega/dy
    def det_jacobian(self, psi_hat):
        dpsi_dy = np.fft.ifft2(1j * self.ky * psi_hat).real
        domega_dx = np.fft.ifft2(1j * self.kx * self.k2 * psi_hat).real
        dpsi_dx = np.fft.ifft2(1j * self.kx * psi_hat).real
        domega_dy = np.fft.ifft2(1j * self.ky * self.k2 * psi_hat).real
...
#ETD-RK4 method
    ...
    def run(self):
        ...
        for index, step in tqdm(enumerate(range(self.num_steps))):
...
# Animation using matplotlib
    def animate_snapshots(self, psis, snapshot_steps):
        ...
        indices = np.linspace(0, len(psis) - 1, snapshot_steps, False, dtype=int)
        ...
        def update(frame):
            ...
            ax.set_title(f'Streamfunction ψ, step {step_numbers[frame] + 1}')
            return (im,)
